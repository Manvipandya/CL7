...
train_data = pd.read_csv('D:/PCA/train.csv')
test_data = pd.read_csv('D:/PCA/test.csv')
...
class NN_2hd:
    ...
    def output_weights(self):
        ...
        return predictions, self.W1, self.W2, self.W3, self.W4, self.b1, self.b2, self.b3, self.b4

model = NN_2hd(x_train[0:800], np.array(y_train[0:800]))
...
prediction, W1, W2, W3, W4, b1, b2, b3, b4 = model.output_weights()
    #if i%100 == 0:
    #    print(i)
    
Z1 = np.dot(x_train[800:], W1) + b1
A1 = np.tanh(Z1)
Z2 = np.dot(A1, W2) + b2
A2 = np.tanh(Z2)
Z3 = np.dot(A2, W3) + b3
...
cm = confusion_matrix(y_train[800:], y_pred)

print(cm)
print("Accuracy : ")
print(((cm[0][0]+ cm[1][1])/91)*100)
